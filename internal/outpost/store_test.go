...
func TestStoreCreatesAndListsOutposts(t *testing.T) {
	ctx := context.Background()
	databasePath := filepath.Join(t.TempDir(), "outpost.db")
	...
	outposts, err := store.List(ctx)
	if err != nil {
		t.Fatalf("List() error = %v", err)
	}

	if want := []Outpost{created}; !reflect.DeepEqual(outposts, want) {
		t.Fatalf("List() = %v, want %v", outposts, want)
	}
}

func TestStoreDeletesOutpostByName(t *testing.T) {
	store, err := Open(context.Background(), filepath.Join(t.TempDir(), "outpost.db"))
	if err != nil {
		t.Fatalf("Open() error = %v", err)
	}
	defer store.Close()
