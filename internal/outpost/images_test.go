...
func TestImageTagsAndReferences(t *testing.T) {
	...
	s, err := Open(ctx, filepath.Join(t.TempDir(), "outpost.db"))
	if err != nil {
		t.Fatal(err)
	}
	defer s.Close()
...
func TestImageDigestBlockedByOutpostAndGC(t *testing.T) {
	ctx := context.Background()
	s, err := Open(ctx, filepath.Join(t.TempDir(), "outpost.db"))
	if err != nil {
		t.Fatal(err)
	...
	if err := s.RemoveImage(ctx, d); err != nil {
		t.Fatalf("deleted outpost still blocked digest: %v", err)
	}
	if err := s.PutImage(ctx, d, 1, ""); err != nil {
		t.Fatal(err)
	}
