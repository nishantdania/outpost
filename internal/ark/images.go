...
func (s *Store) RemoveImage(ctx context.Context, ref string) error {
	...
	if err = tx.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM arks WHERE image_id=? AND desired_state != 'deleted')`, d).Scan(&used); err != nil {
		return err
	}
	if used {
...
func (s *Store) GarbageCollectImages(ctx context.Context) ([]string, error) {
	...
	defer tx.Rollback()
	rows, err := tx.QueryContext(ctx, `SELECT digest FROM images WHERE digest NOT IN (SELECT digest FROM image_tags) AND digest NOT IN (SELECT image_id FROM arks WHERE desired_state != 'deleted')`)
	if err != nil {
		return nil, err
	}
	var ids []string
